...
academic_years = pd.DataFrame([
    {"id": 1, "name": "2025-2026", "start_date": "2025-09-01", "end_date": "2026-06-30", "is_current": True}
])

programs = pd.DataFrame([
    {"id": 1, "name": "B3 Data & IA", "department": "Tech", "degree_level": "B3", "schedule_type": "Cours du jour"},
    {"id": 2, "name": "B3 DevOps", "department": "Tech", "degree_level": "B3", "schedule_type": "Cours du jour"},
    {"id": 3, "name": "M1 Tech & Business", "department": "Business", "degree_level": "M1", "schedule_type": "Cours du soir"}
])

cohorts = pd.DataFrame([
    {"id": 1, "program_id": 1, "academic_year_id": 1, "name": "B3 Data & IA - G1"},
    {"id": 2, "program_id": 2, "academic_year_id": 1, "name": "B3 DevOps - G1"},
    {"id": 3, "program_id": 3, "academic_year_id": 1, "name": "M1 Tech & Biz - Soir"}
])

modules = pd.DataFrame([
    {"id": 1, "code": "DATA-301", "name": "Machine Learning Fundamentals", "credits": 5, "description": "Intro to ML"},
    {"id": 2, "code": "DATA-302", "name": "Advanced Python for Data", "credits": 4, "description": "Pandas, NumPy"},
    {"id": 3, "code": "DEV-301", "name": "Go Microservices", "credits": 5, "description": "Backend architecture"},
    {"id": 4, "code": "SYS-301", "name": "Cloud Infrastructure", "credits": 4, "description": "AWS & Docker"}
])
...
for i in range(1, total_users + 1):
    role = "student" if i <= num_students else ("teacher" if i <= num_students + num_teachers else "admin")
    users_data.append({
        "id": i,
...
for i, user in users[users['role'] == 'teacher'].iterrows():
    teachers_data.append({
        ...
        "user_id": user['id'],
        "specialty": random.choice(["Data Science", "Backend", "Cloud Ops", "Mathematics"])
    })
...
for i, user in users[users['role'] == 'student'].iterrows():
    ...
    students_data.append({
        "id": s_id,
        "user_id": user['id'],
        "student_id_number": f"STU-{random.randint(10000, 99999)}",
        "birth_date": fake.date_of_birth(minimum_age=18, maximum_age=25),
        "cohort_id": cohort_id,
        "enrollment_status": random.choices(["ACTIVE", "SUSPENDED", "DROPPED"], weights=[90, 5, 5])[0],
        "created_at": datetime.now()
    })
    student_traits[s_id] = {
...
for cm in cohort_modules_data:
    evaluations_data.append({"id": eval_id, "cohort_module_id": cm["id"], "title": "Midterm Exam", "type": "Exam", "weight_percentage": 40.0, "max_score": 20.0, "evaluation_date": "2025-11-15"})
    eval_id += 1
    evaluations_data.append({"id": eval_id, "cohort_module_id": cm["id"], "title": "Final Project", "type": "Project", "weight_percentage": 60.0, "max_score": 20.0, "evaluation_date": "2026-01-20"})
    eval_id += 1
...
for eval_record in evaluations_data:
    c_mod = next(item for item in cohort_modules_data if item["id"] == eval_record["cohort_module_id"])
